...
// start/end/length. These are cAtomically updated so it works as a
// circular buffer.
// Since the only control primitive that can be used is cAtomicAdd(),
...
// since we do not even have an cAtomic primitive.
...
__device__ volatile static char *printfBufferPtr = NULL;     // Current cAtomically-incremented non-wrapped offset
...
// That means sizeof(float) in this case, but we'll use (long long) for
...
//  cAtomic function to ensure that it's ours. We handle wrapping
...
__device__ static char *getNextPrintfBufPtr()
{
    ...
    // For sm_10 architectures, we have no cAtomic add - this means we must split the
...
#else
    // Much easier with an cAtomic operation!
...
//  the display can distinguish between float and float), and the
    ...
    while(p != NULL)
    {
        ...
        *p = '\0';
        switch(specifier)
        {
            ...
            // These all take float arguments
            case 'e':
            case 'E':
            case 'f':
            case 'g':
